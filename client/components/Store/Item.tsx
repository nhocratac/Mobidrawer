...
const LiItem = ({
    ...
    return (
        <li className="flex flex-col gap-1 border-b pb-2 mx-auto w-full max-w-[280px]">
            <Link href={`${path.store}/${id}`} className="w-full h-auto">
                <div className="w-full h-auto aspect-[4/3] group relative">
                    {previewImageUrl && <Image width={280} height={420} src={previewImageUrl} alt={`template-${name}`} className="w-full h-full object-cover rounded-[8px]" />}
